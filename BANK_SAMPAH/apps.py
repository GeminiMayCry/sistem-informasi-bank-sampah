...
from streamlit_gsheets import GSheetsConnection  # <-- Library Koneksi Google Sheets
...
# Tangani jika file secrets.toml belum dibuat sama sekali
try:
...
def check_database():
    """Mengecek apakah konfigurasi URL spreadsheet tersedia"""
    return bool(SPREADSHEET_URL)
...
def ensure_columns(df, sheet_name):
    ...
    for col in required:
        if col not in df.columns:
            if col in ["STATUS PEMBAYARAN"]:
                df[col] = "BELUM LUNAS"
            elif col in ["POTONGAN KAS", "SALDO NASABAH"]:
                df[col] = 0
            else:
                df[col] = ""
    return df
...
def load_sheet(sheet_name):
    """Membaca data dari tab worksheet Google Sheets"""
    try:
        # ttl=0 memastikan data yang dibaca selalu paling baru secara real-time
        ...
        df = ensure_columns(df, sheet_name)

        # Formatting Tipe Data Teks
        text_columns = [
            "NIK", "NAMA", "RT", "RW", "ALAMAT", "HP", "REKENING",
            "ITEM_ID", "ITEM", "JENIS", "SATUAN",
            "TRANSAKSI_ID", "DETAIL_ID", "PENJUALAN_ID",
            "MUTASI_ID", "JENIS_MUTASI", "REFERENSI",
            "KEUANGAN_ID", "KETERANGAN", "STATUS PEMBAYARAN"
        ]
        for col in text_columns:
            if col in df.columns:
                df[col] = df[col].fillna("").astype(str).str.strip()

        # Formatting Tipe Data Angka
        numeric_columns = [
            "KG", "HARGA", "SUBTOTAL", "TOTAL KG", "TOTAL HARGA",
            "POTONGAN KAS", "SALDO NASABAH", "SUDAH DIBAYAR", "SISA SALDO",
            "HARGA SETOR", "DEBIT", "KREDIT", "NO"
        ]
        for col in numeric_columns:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

        # Formatting Tipe Data Tanggal
        date_columns = [
            "TANGGAL", "TANGGAL TRANSAKSI", "TANGGAL PENDAFTARAN",
            "TANGGAL MULAI BERLAKU", "TANGGAL LUNAS"
        ]
        ...
        st.error(f"Gagal membaca worksheet '{sheet_name}': {e}")
        return pd.DataFrame(columns=REQUIRED_COLUMNS.get(sheet_name, []))
...
def save_sheet(sheet_name, df):
    """Menyimpan satu worksheet ke Google Sheets"""
    try:
        df_save = df.copy()
        # Format tanggal menjadi string YYYY-MM-DD sebelum dikirim
        for col in df_save.select_dtypes(include=['datetime64[ns]', 'datetime']).columns:
            df_save[col] = df_save[col].dt.strftime('%Y-%m-%d').fillna("")
            
        conn.update(spreadsheet=SPREADSHEET_URL, worksheet=sheet_name, data=df_save)
        st.cache_data.clear()
        return True
    except Exception as e:
...
def save_multiple_sheets(sheet_data):
    """Menyimpan beberapa worksheet sekaligus (transaksi) ke Google Sheets"""
    try:
                            ...
                            if len(idx_list) > 0:
                                ...
                                if sisa_saldo > 0:
                                    ...
                                    new_finance = pd.DataFrame([[
                                        ...
                                        0,           # DEBIT = 0 (Bukan Pemasukan)
                                        sisa_saldo,  # KREDIT = sisa_saldo (Pengeluaran Kas/Uang Keluar)
                                        trx_id
                                    ]], columns=REQUIRED_COLUMNS[SHEET_KEUANGAN])
                                    df_keuangan = pd.concat([df_keuangan, new_finance], ignore_index=True)
...
def main():
    ...
    if not check_database():
        st.error("Konfigurasi Google Sheets tidak ditemukan.")
